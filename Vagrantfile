...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  #  However, this should be changed to :windows for Windows guests.
  #config.vm.guest = ":linux"             # change to ':windows' for MS Windows OS
  ...
  #
  # changes here may requre changes to your /etc/mongo.conf file
  #config.vm.network "forwarded_port", guest: 1884, host: 1884, host_ip: "127.0.0.1" # port for websocket
  #config.vm.network "forwarded_port", guest: 1883, host: 1883, host_ip: "127.0.0.1" # port for mqtt
  ...
  #config.vm.network "forwarded_port", guest: 6006, host: 6006, host_ip: "127.0.0.1" # port for tensorboard
  ...
  #config.vm.synced_folder "/home/jeff/src/computer-vision", "/home/vagrant/computer-vision", owner: "vagrant", group: "vagrant", mount_options: ["dmode=775,fmode=664"]
  #config.vm.synced_folder "./blog", "/home/vagrant/blog", :mount_options => ["dmode=777,fmode=666"]
  #config.vm.synced_folder "./", "/home/vagrant", owner: "vagrant", group: "vagrant", mount_options: ["dmode=775,fmode=664"]

  #config.vm.synced_folder "~/src/vagrant-machines/jetson-dev/shared-data", "/home/vagrant/shared-data", owner: "vagrant", group: "vagrant", mount_options: ["dmode=775,fmode=774"]
  #config.vm.synced_folder "~/src/vagrant-machines/jetson-dev/shared-data", "/home/vagrant/shared-data", mount_options: ["dmode=777,fmode=666"
  #config.vm.synced_folder "~/src/vagrant-machines/jetson-dev/shared-data", "/home/vagrant/shared-data", owner: "vagrant", group: "vagrant"

  #config.vm.synced_folder "/home/jeff/src/vagrant-machines/jetson-dev/shared-data", "/home/vagrant/shared-data", owner: "vagrant", group: "vagrant"
  #config.vm.synced_folder "./shared-data", "/home/vagrant/shared-data", owner: "vagrant", group: "vagrant"
...
SHELL
  ...
  ## ---------- create your user GUI desktop environment (as vagrant) -----------
  #config.vm.provision "shell", name: "create your user desktop environment (as vagrant)", run: "once", inline: <<-SHELL
    ## become the vagrant user
    #su --login vagrant --shell /bin/bash <<'EOF'

    ## setup your autostart directory
    #mkdir ~/.config
    #mkdir ~/.config/autostart

    ## load content into autostart file
    #cat <<'ENDofFILE' > ~/.config/autostart/VboxClipboard.desktop
    #[Desktop Entry]
    #Type=Application
